...
public class WXBizMsgCrypt {
    ...
    public WXBizMsgCrypt(String token, String encodingAesKey, String receiveId) {
        if (encodingAesKey.length() != 43) {
            WXBizMsgCrypt.log.error(new AesException(AesException.IllegalAesKey).getMessage());
        }
    ...
    public String encrypt(String randomStr, String text) throws AesException {
        ...
        byte[] randomStrBytes = randomStr.getBytes(WXBizMsgCrypt.CHARSET);
        byte[] textBytes = text.getBytes(WXBizMsgCrypt.CHARSET);
        byte[] networkBytesOrder = getNetworkBytesOrder(textBytes.length);
        byte[] receiveidBytes = receiveId.getBytes(WXBizMsgCrypt.CHARSET);
    ...
    public String decrypt(String text) throws AesException {
        ...
        try {
            ...
            xmlContent = new String(Arrays.copyOfRange(bytes, 20, 20 + xmlLength), WXBizMsgCrypt.CHARSET);
            from_receiveid = new String(Arrays.copyOfRange(bytes, 20 + xmlLength, bytes.length),
                    WXBizMsgCrypt.CHARSET);
        } catch (Exception e) {
            e.printStackTrace();
